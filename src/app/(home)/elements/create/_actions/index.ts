...
import { ElementOption } from '../_components/ElementForm'
import { z } from 'zod'
...
async function educationAction(data: any): Promise<IActionResponse> {
    ...
    const schema = z.object({
        customId: z.string().min(3, 'Id need in the minimun 3 characteres'),
        description: z.string().min(15, 'Description need in the minimun 15 characteres'),
        finishPeriod: z.date({ message: 'Finish period need be in datetime format' }),
        ...
        startPeriod: z.date({ message: 'Start period need be in datetime format' }),
    })
...
async function personalInformationAction(data: any): Promise<IActionResponse> {
    ...
    const schema = z.object({
        ...
        github: z.string().startsWith('https://github.com/', 'Github link needs start with https://github.com/'),
        jobTitle: z.string().min(5, 'Job title needs in the minimun 5 characteres'),
        linkedin: z.string().startsWith('https://www.linkedin.com/', 'LinkedIn link needs start with https://www.linkedin.com/'),
