...
export default function ElementHandleOptionsComponent({
    ...
    const [visibleViewPopup, setVisibleViewPopup] = useState(false)
    const [visibleDeletePopup, setVisibleDeletePopup] = useState(false)

    async function clientRemoveFunction(){

        const response = await removeFunction(elementId)
    ...
    >
        { visibleViewPopup && <Popup
            hideFunction={() => setVisibleViewPopup(false)}
        >
            { PopupElement }
        </Popup> }
        ...
        >
            <p>Have you sure to delete the element?</p>
        ...
        <svg 
            ...
            onClick={() => setVisibleViewPopup(true)}
        ...
        <svg 
            className='cursor-pointer'
            onClick={() => router.push(`/elements/edit/${elementId}/${elementType}`)}
